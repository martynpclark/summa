...
for outputFile in $( ls  ${outputPath}${expID}/*${expID}*spinup*${expName}.nc ); do
 ...
 inputName=${inputPath}${siteName}Fluxnet.1.4_met.nc

 # get information about the time dimension
 timeInfo=`ncks -C -v time -m $inputName | grep "size =" | grep dimension`

 # get the number of time steps in the forcing datafile
 IFS='=' read -a strarr0 <<< "${timeInfo}"
 IFS=' ' read -a strarr1 <<< "${strarr0[-1]}"
 numberInputSteps=${strarr1[0]}
