...
contains
  ...
  subroutine evaluate_objective(comm, rank, nproc,        &
    ...
    ! NOTE: iulog is switched to stderr further below, once observations confirm this is a
    !       calibration run, so that the objective value alone reaches stdout
    ...
    hasObs = .false.
    if(allocated(summa1_struc(n)%obs%obs_file))then
      if(len_trim(summa1_struc(n)%obs%obs_file) > 0) hasObs = .true.
    endif
