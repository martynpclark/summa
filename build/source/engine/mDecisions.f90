...
subroutine mDecisions(err,message)
  ...
  if (.not.ngen_active)then
    if(isPrint) write(iulog,'(a,i4,1x,4(i2,1x))') 'startTime: iyyy, im, id, ih, imin = ', startTime%var(1:5)
    if(isPrint) write(iulog,'(a,i4,1x,4(i2,1x))') 'finshTime: iyyy, im, id, ih, imin = ', finshTime%var(1:5)
  endif
  ...
  select case(model_decisions(iLookDECISIONS%hc_profile)%iDecision)
    ...
    case(constant)
      if(model_decisions(iLookDECISIONS%infRateMax)%iDecision == topModel_GA)then
        write(iulog,*) 'DEPRECATION WARNING: infRateMax topmodel_GA evaluates the hydraulic conductivity at the wetting front, but hc_profile is constant so the column is homogeneous. Please use GreenAmpt instead (set "infRateMax" to "GreenAmpt" in model decisions input file)'
      end if
  end select
...
subroutine readoption(err,message)
  ...
  if (.not.ngen_active)then
    if(isPrint) write(iulog,'(2(a,1x))') 'decisions file = ', trim(infile)
  endif
  ! open file
  ...
  do iDecision=1,nDecisions
    ...
    if (.not.ngen_active)then
      if(isPrint) write(iulog,'(i4,1x,a)') iDecision, trim(option)//': '//trim(decision)
    endif
