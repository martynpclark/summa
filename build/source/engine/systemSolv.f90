...
contains
 ...
 integer(i4b)                    :: nSoilLiq                     ! number of liquid water state variables in soil layers
 integer(i4b)                    :: nSoilMat                     ! number of matric head state variables in soil layers
 ...
 integer(i4b)                    :: nLiqState                    ! number of liquid water state variables
 ...
 integer(i4b),allocatable        :: ixNrgOnlyAll(:)              ! list of indices for the energy states in the full state vector 
 integer(i4b),allocatable        :: ixLiqOnlyAll(:)              ! list of indices for the liquid water states in the full state vector 
 integer(i4b),allocatable        :: ixMatOnlyAll(:)              ! list of indices for the matric head states in the full state vector 
 integer(i4b),allocatable        :: ixWatOnlyAll(:)              ! list of indices for the mass states in the full state vector
 integer(i4b),allocatable        :: ixLiqOnlySoil(:)             ! list of indices for the liquid water states in the soil vector
 integer(i4b),allocatable        :: ixMatOnlySoil(:)             ! list of indices for the matric head states in the soil vector
 ...
 real(dp),allocatable            :: oldJac(:,:)                  ! copy of the analytical Jacobian matrix before it is decomposed in lapack routines
 ...
 real(dp)                        :: newtBias                     ! (1 - dogNewtBias) + dogNewtBias*xGamma   ! [xGamma,1] = bias towards the newton step
 real(dp)                        :: steepLen                     ! length of the steepest descent step
 ...
 integer(i4b),parameter          :: newtonScaled=2               ! named variable to denote that we accepted the newton step within the trust region
 integer(i4b),parameter          :: newtonInside=3               ! named variable to denote that we accepted the newton step within the trust region
 type(action),parameter,dimension(0:3) :: stepDescription=(/action(0,'dogleg'),action(1,'steepScaled'),action(2,'newtonScaled'),action(3,'newtonInside')/)
 ...
 logical(lgt)                    :: feasibleSolution             ! flag to denote if the solution is feasible
 logical(lgt)                    :: trustRegionCollapse          ! flag to denote if the trust region has collapsed (tRadius < tRadiusMin) 
 logical(lgt)                    :: trustRegionStagnate          ! flag to denote if the trust region is stagnant (radius < trustStagnate over the past nStagnate iterations)
 integer(i4b)                    :: trustResult=4                ! result of the trust region refinement 
 integer(i4b),parameter          :: unfeasible=-1                ! step rejected (unfeasible function evaluation)
 ...
 type(action),parameter,dimension(-1:4) :: trustDescription=(/action(-1,'unfeasible'),action(0,'stepRejected'),action(1,'shrinkTrust'),action(2,'expandTrust'),action(3,'fixedTrust'),action(4,'uninitialized')/)
 real(dp)                        :: tRadius                      ! trust region radius
 real(dp)                        :: fPred                        ! predicted function value, scaled (-)
 ...
 ! implement a watchdog strategy for non-monotone sequence of function evaluations
 integer(i4b),parameter          :: nWatchdog=3                  ! number of previous function evaluations to check
 real(dp),dimension(0:nWatchdog) :: fxHistory                    ! history of function evaluations
 real(dp)                        :: fOldWatchdog                 ! maximum function evaluation for the previous nWatchdog iterations
 ! check for trust region stagnation
 integer(i4b),parameter          :: nStagnate=10                 ! number of iterations where the trust region does not rise above a specified value
 real(dp),dimension(0:nStagnate) :: trustHistory                 ! history of trust region radius
 ! check convergence
 ...
 ! ------------------------------------------------------------------------------------------------------
 real(dp)                        :: switchMatricHead=0.99_dp     ! relative switching tolerance
 real(dp)                        :: switchVolFracLiq=0.89_dp     ! relative switching tolerance
 real(dp)                        :: matricHeadSwitchTol          ! actual switching tolerance for matric head
 real(dp)                        :: volFracLiqSwitchTol          ! actual switching tolerance for volumetric fraction of liquid water
 ...
 mLayerVolFracWatInit(nSnow+1:nLayers) = mLayerVolFracLiq(nSnow+1:nLayers) + mLayerVolFracIce(nSnow+1:nLayers)

 ! initialize the primary variable switching tolerances
 matricHeadSwitchTol = switchMatricHead*theta_sat
 volFracLiqSwitchTol = switchVolFracLiq*theta_sat

 ! define the state type for the mass in the soil domain
 do iState=1,nSoil
  if(mLayerVolFracWatInit(nSnow+iState) > matricHeadSwitchTol)then
   ixSoilMassVar(iState) = ixMatState
  else
   ixSoilMassVar(iState) = ixLiqState
  endif
 end do
 ...
 ! check
 if(printFlag)then
  write(*,'(a,1x,f25.10)') 'scalarCanairTemp = ', scalarCanairTemp
  write(*,'(a,1x,f25.10)') 'scalarCanopyTemp = ', scalarCanopyTemp
 endif
 ...
 ! define the number of liquid water and matric head state variables in the soil layers
 nSoilLiq = count(ixSoilMassVar==ixLiqState)
 nSoilMat = count(ixSoilMassVar==ixMatState)

 ! define the number state variables of different type
 nNrgState = nVegNrg + nLayers  ! number of energy state variables
 nLiqState = nSnow + nSoilLiq   ! number of liquid water state variables (NOTE: exclude canopy water)
 ...
 select case(ixSolve)
  case(ixFullMatrix); allocate(aJac(nState,nState),aJacScaled(nState,nState),oldJac(nState,nState),stat=err)
  case(ixBandMatrix); allocate(aJac(nBands,nState),aJacScaled(nBands+kl,nState),oldJac(nBands+kl,nState),stat=err)  ! +kl for scaled Jacobian to create space for the band-diagonal solver
 ...
 allocate(ixNrgOnlyAll(nNrgState),ixLiqOnlyAll(nLiqState),ixMatOnlyAll(nSoilMat),ixWatOnlyAll(nLayers+nVegLiq),ixLiqOnlySoil(nSoilLiq),ixMatOnlySoil(nSoilMat),stat=err)
 ...
 allocate(dMat(nState),sMul(nState),rAdd(nState),fluxVec0(nState),rVec(nState),rhs(nState,nRHS),iPiv(nState),stat=err)
 ...
 ixStateType(ixSoilOnlyMat) = ixSoilMassVar(:) ! (can be either ixLiqState or ixMatState)
 ...
 ! define the indices of specific state variables types within the full state vector
 ixNrgOnlyAll  = pack(ixAllState,(ixStateType==ixNrgState))
 ixLiqOnlyAll  = pack(ixAllState,(ixStateType==ixLiqState))
 ixMatOnlyAll  = pack(ixAllState,(ixStateType==ixMatState))
 ixWatOnlyAll  = pack(ixAllState,(ixStateType==ixMassState .or. ixStateType==ixLiqState .or. ixStateType==ixMatState))

 ! define the indices of specific state variables within the soil vector
 ixLiqOnlySoil = pack(ixSoilState,(ixSoilMassVar==ixLiqState))
 ixMatOnlySoil = pack(ixSoilState,(ixSoilMassVar==ixMatState))

 ! check indices
 if(printFlag)then
  write(*,'(a,1x,100(i2,1x))') 'ixNrgOnlyAll = ', ixNrgOnlyAll
  write(*,'(a,1x,100(i2,1x))') 'ixLiqOnlyAll = ', ixLiqOnlyAll
  write(*,'(a,1x,100(i2,1x))') 'ixMatOnlyAll = ', ixMatOnlyAll
  write(*,'(a,1x,100(i2,1x))') 'ixWatOnlyAll = ', ixWatOnlyAll
 endif
 ...
 ! build the state vector for energy in the snow and soil domain
 stateVecInit(ixSnowSoilNrg) = mLayerTemp(1:nLayers)

 ! build the state vector for volumetric liquid water content in the snow and soil domain
 if(nLiqState>0) stateVecInit(ixLiqOnlyAll) = mLayerVolFracWatInit(ixLiqOnlySoil)
 if(nSoilMat >0) stateVecInit(ixMatOnlyAll) = mLayerMatricHead(ixMatOnlySoil)

 if(printFlag)then
  write(*,'(a,1x,10(f17.8,1x))') 'stateVecInit = ', stateVecInit
 endif
 ...
 select case(ixNrgFormulation)
  case(temperatureState); xScaleFactor(ixNrgOnlyAll) = xScaleTemp      ! (K)
  case(enthalpyState);    xScaleFactor(ixNrgOnlyAll) = xScaleNrg       ! (J m-3)
 ...
 if(nLiqState > 0)  xScaleFactor(ixLiqOnlyAll) = xScaleLiq         ! (-)
 if(nSoilMat > 0)   xScaleFactor(ixMatOnlyAll) = xScaleMat         ! (m)
 ...
 if(ixFunctionScaling==userDefined)then
  ! (energy)
  fScale(ixNrgOnlyAll) = 1._dp / fScaleNrg   ! (J m-3)-1
  ! (mass)
 ...
 ! enable re-starts if things get stuck
 multipleRestarts: do iRestart=1,maxRestarts

  ! initialize history for the function and trust region radius
  fxHistory(:)    = valueMissing
  trustHistory(:) = valueMissing

  ! iterate
  iterating: do iter=1,maxiter

   ! keep track of the number of iterations
   niter = iter+1  ! +1 because of the function call earlier

   ! identify the primary variable in each node that is used for the solution of Richards' equation
   ...
   call computeJacobian(aJac,err,cmessage)
   if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)
   ...
   ! save the function evaluation
   fxHistory    = eoshift(fxHistory,shift=-1,boundary=valueMissing)  ! discard earliest function value
   fxHistory(0) = fOld
   fOldWatchdog = maxval(fxHistory)
   if(printFlag)then
    print*, 'fxHistory = ', fxHistory
    print*, 'fOldWatchdog = ', fOldWatchdog
   endif
   ...
   !newtStep(ixNrgOnlyAll)  = newtStepOrig(ixNrgOnlyAll)
   !newtStep(ixLiqOnlyAll)  = newtStepOrig(ixLiqOnlyAll)
   ...
   if(converged) exit multipleRestarts
   ...
   if(ixIterRefinement==trustRegion)then
    ...
    trustRegionStagnate = (maxval(trustHistory) < trustStagnate)
    ! (check if the trust region history has collapsed or is is stagnant)
    if(trustRegionCollapse .or. trustRegionStagnate) exit iterating  ! re-start iterations
   endif ! if trust region method

  end do iterating

  ! print progress
  if(printFlag) print*, '*** another trial'

  ! check that we converged
  if(iRestart==maxRestarts)then
   message=trim(message)//'failed to converge';
   err=-20;return
  endif

 end do multipleRestarts
 ...
 deallocate(ixStateType,ixNrgOnlyAll,ixLiqOnlyAll,ixMatOnlyAll,ixWatOnlyAll,ixLiqOnlySoil,ixMatOnlySoil,ixAllState,stateVecInit,stateVecTrial,stateVecNew, stat=err)
 ...
 contains
  ...
  implicit none
  real(dp),intent(in)  :: stateVec(:)      ! state vector (mixed units)
  logical(lgt)         :: check            ! flag to denote if the solution is feasible
  logical(lgt)         :: feasible         ! flag to denote if the solution is feasible
  ...
  if(any(abs(xInc(ixNrgOnlyAll)) > zMaxTempIncrement))then
   iMax       = maxloc( abs(xInc(ixNrgOnlyAll)) )                     ! index of maximum temperature increment
   xIncFactor = abs( zMaxTempIncrement/xInc(ixNrgOnlyAll(iMax(1))) )  ! scaling factor for the iteration increment (-)
  ...
  if(ixNrgFormulation==enthalpyState)then
   xInc(ixNrgOnlyAll) = xInc(ixNrgOnlyAll)/dMat(ixNrgOnlyAll)
  endif
  if(printFlag)then
   print*, 'iter = ', iter
  ...
  stateVecNew(ixNrgOnlyAll) = stateVecTrial(ixNrgOnlyAll) + xInc(ixNrgOnlyAll)
  ...
  if(printFlag)then
   write(*,'(a,1x,10(f20.10,1x))') 'vThetaInit(1:nSoil) = ',  vThetaInit(1:nSoil)
   write(*,'(a,1x,10(f20.10,1x))') 'vThetaTrial(1:nSoil) = ', vThetaTrial(1:nSoil)
   write(*,'(a,1x,10(f20.10,1x))') 'rVec(ixSoilOnlyMat)  = ', (rVec(ixSoilOnlyMat(iState)),iState=1,nSoil)
  endif
  ...
  do iLayer=1,nLayers
   ...
   if(printFlag)then
    if(iLayer < 3) write(*,'(a,1x,i4,1x,10(f25.15,1x))') 'iLayer, mLayerTempTrial(iLayer), iLayerNrgFlux(iLayer-1:iLayer), ssdNetNrgFlux(iLayer)   = ', &
                                                          iLayer, mLayerTempTrial(iLayer), iLayerNrgFlux(iLayer-1:iLayer), ssdNetNrgFlux(iLayer)
   endif
  end do
  ...
  ! *********************************************************************************************************
  subroutine computeJacobian(aJac,err,message)
  ! dummy variables
  ...
  select case(ixSolve)
   case(ixFullMatrix); call analJacob(aJac,err,cmessage)
   case(ixBandMatrix); call cpactBand(aJac,err,cmessage)
   case default; err=20; message=trim(message)//'unable to identify option for the type of matrix'
  end select
  if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)
  ...
  if(numericalJacobian)then
   ...
   if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)
   aJac = nJac  ! use the numerical Jacobian
   printFlag=printFlagInit
  ...
  ! *********************************************************************************************************
  subroutine cpactBand(aJac,err,message)
  ! dummy variables
  real(dp),intent(out)           :: aJac(:,:)               ! Jacobian matrix
  integer(i4b),intent(out)       :: err                     ! error code
  character(*),intent(out)       :: message                 ! error message
  ...
  if(computeVegFlux)then  ! (derivatives only defined when vegetation protrudes over the surface)
   ...
   aJac(ixDiag,ixVegWat) = -fracLiqVeg*(dCanopyEvaporation_dCanLiq - scalarCanopyLiqDeriv)*dt !+ dMat(ixVegWat)                    ! ixVegWat: CORRECT
  ...
  ! *********************************************************************************************************
  subroutine analJacob(aJac,err,message)
  implicit none
  ! dummy variables
  real(dp),intent(out)           :: aJac(:,:)               ! Jacobian matrix
  integer(i4b),intent(out)       :: err                     ! error code
  ...
  do iLayer=1,nSoil    ! loop through layers in the soil domain
   ...
   if(nSnow==0 .and. iLayer==1)then  ! upper-most soil layer
    if(computeVegFlux)then
     ...
     if(printFlag) print*, 'aJac(jLayer,ixVegNrg) = ', aJac(jLayer,ixVegNrg)
    endif
  ...
  do iJac=1,nState
   ...
   if(ixNumType==ixNumRes)then ! switch between the residual-based form and flux-based form
    ...
    if(iJac==ixTopNrg)then
     write(*,'(a,1x,10(f20.10,1x))') 'stateVecPerturbed(ixTopNrg), fluxVecJac(ixTopNrg), nJac(ixTopNrg,iJac), fluxVecJac(iJac), fluxVec(iJac) = ', &
                                      stateVecPerturbed(ixTopNrg), fluxVecJac(ixTopNrg), nJac(ixTopNrg,iJac), fluxVecJac(iJac), fluxVec(iJac)
    endif
  ...
  write(*,'(a4,1x,100(i12,1x))') 'xCol', (iLayer, iLayer=iJac1,iJac2)
  do iLayer=iJac1,iJac2; write(*,'(i4,1x,100(e12.5,1x))') iLayer, nJac(iJac1:iJac2,iLayer); end do
  ...
  ! *********************************************************************************************************
  subroutine computeGradient(aJacScaled,rVecScaled,gradScaled,grad,err,message)
  implicit none
  ! input
  real(dp),intent(in)            :: aJacScaled(:,:)  ! scaled jacobian matrix
  real(dp),intent(in)            :: rVecScaled(:)    ! scaled residual vector
  ! output
  real(dp),intent(out)           :: gradScaled(:)    ! scaled gradient
  ...
  ! Sun, W., 2004: Nonmonotone trust region method for solving optimization problems.
  !  Applied Mathematics and Computation, 156, 159-174.
  ...
  if(printFlag)then
   write(*,'(a,1x,10(e17.5,1x))') 'newtLen (scaled)    = ', newtNorm
   write(*,'(a,1x,10(e17.5,1x))') 'newtStep (scaled)   = ', newtStep(iJac1:iJac2)
  endif

  ! initialize the trust region radius
  if(iter==1) tRadius = newtNorm*multiplyNewt

  ! initialize the trust region result
  trustResult = unInitialized
  ...
  ! *********************************************************************************************************
  ! internal subroutine trustInitialize: compute gradient, cauchy point, norms, etc... 
  ! *********************************************************************************************************
  subroutine trustInitialize(err,message)
  implicit none
  ! dummy
  integer(i4b),intent(out)       :: err           ! error code
  character(*),intent(out)       :: message       ! error message
  ...
  ! compute the x-section along the negative gradient
  if(doGradXsection)then
   call gradXsection(stateVecTrial,gradScaled,err,cmessage)
   if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)
  endif

  ! compute the step in the direction of steepest descent (to the point where the quadratic starts increasing)
  select case(ixSolve) ! check if full Jacobian or band-diagonal matrix
   case(ixFullMatrix); tempVec = matmul(oldJac,gradScaled)  ! full Jacobian matrix
   case(ixBandMatrix) ! band-diagonal matrix
    tempVec(:) = 0._dp
    do iJac=1,nState  ! (loop through state variables)
     do iState=max(1,iJac-ku),min(nState,iJac+kl)
      tempVec(iState) = tempVec(iState) + gradScaled(iJac)*oldJac(kl+ku+1+iState-iJac,iJac)
     end do
    end do
   case default; err=20; message=trim(message)//'unable to identify option for the type of matrix'
  end select  ! (option to solve the linear system A.X=B)
  ...
  beta      = dot_product(tempVec,tempVec)  ! beta variable in algorithm 6.4.4 (double dogstep) in Dennis and Schnabel 1996
  steepLen  = gradNorm2/beta
  steepStep = -gradScaled*steepLen
  if(printFlag)then
   write(*,'(a,1x,10(e17.5,1x))') 'steepLen (scaled)    = ', steepLen
  ...
  ! *********************************************************************************************************
  ! internal subroutine trustIncrement: compute the iteration increment
  ! *********************************************************************************************************
  subroutine trustIncrement(err,message)
  implicit none
  ! dummy
  integer(i4b),intent(out)       :: err           ! error code
  character(*),intent(out)       :: message       ! error message
  ...
  ! Case 2: steepest descent step outside trust region
  elseif(steepNorm > tRadius)then
   xIncScaled = steepStep * tRadius/steepNorm ! step to the trust region radius in the direction of steepest descent
  ...
  ! case 3: steepest descent step inside and newton step outside
   ...
   if(printFlag) then
    ...
    write(*,'(a,1x,10(e20.10,1x))') 'biasNewtStep = ', biasNewtStep(iJac1:iJac2)
    write(*,'(a,1x,10(e20.10,1x))') 'steepStep    = ', steepStep(iJac1:iJac2)
    write(*,'(a,1x,10(e20.10,1x))') 'xIncScaled   = ', xIncScaled(iJac1:iJac2)
   endif
  ...
  ! *********************************************************************************************************
  ! internal subroutine gradXsection: take a cross section of the obf func in direction of negative gradient
  ! *********************************************************************************************************
  subroutine gradXsection(stateVecTrial,gradScaled,err,message)
  implicit none
  ! input
  real(dp),intent(in)            :: stateVecTrial(:)          ! full model state vector (mixed units)
  real(dp),intent(in)            :: gradScaled(:)             ! scaled gradient
  ! output
  integer(i4b),intent(out)       :: err                       ! error code
  character(*),intent(out)       :: message                   ! error message
  ! local
  integer(i4b)                   :: jTrial                    ! loop over the x-section
  real(dp), dimension(nLayers)   :: mLayerVolFracLiqLocal     ! volumetric fraction of liquid water (-)
  real(dp), dimension(nLayers)   :: mLayerVolFracIceLocal     ! volumetric fraction of ice (-)
  real(dp), dimension(nLayers)   :: mLayerVolFracWatLocal     ! volumetric fraction of total water (-)
  ...
  real(qp)                       :: rVecTemp(nState)          ! temporary residual vector (NOTE: quadruple precision)
  real(dp)                       :: rVecScaled(nState)        ! scaled residual vector
  real(dp)                       :: xLambda                   ! step length
  real(dp)                       :: fActual                   ! actual func eval
  ! initialize error control
  ...
  do jTrial=1,100
   ...
   ! define the iteration increment
   xLambda  = real(jTrial, dp)/10000._dp
   ...
   if(ixNrgFormulation==enthalpyState)then
    ...
    xDiff(ixSnowSoilNrg) = xDiff(ixSnowSoilNrg)/dMat(ixSnowSoilNrg)
   endif

   ! define the new state vector
   stateVecNew = stateVecTrial + xDiff

   ! check if the state vector is feasible
   if(.not.feasible(stateVecNew)) cycle
   ...
   call xFluxResid(&
                   ...
                   ! output
                   fluxVec0,              & ! intent(out): flux vector (mixed units)
                   rVecTemp,              & ! intent(out): residual vector (mixed units)
                   err,cmessage)            ! intent(out): error code and error message
   if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)

   ! compute the actual function evaluation
   rVecScaled(1:nState) = fScale(1:nState)*real(rVecTemp(1:nState), dp)   ! scale the residual vector (NOTE: residual vector is in quadruple precision)
   fActual=0.5_dp*dot_product(rVecScaled,rVecScaled)
   write(*,'(a,1x,10(f20.10,1x))') 'steepLen, xLambda, fOld, fActual = ', steepLen, xLambda, fOld, fActual
  ...
  ! *********************************************************************************************************
  ! internal subroutine gridObjfunc: grid the objective function
  ! *********************************************************************************************************
  subroutine gridObjFunc(stateVecTrial,outputFilename,err,message)
  implicit none
  ! input
  real(dp),intent(in)            :: stateVecTrial(:)          ! full model state vector (mixed units)
  character(*),intent(in)        :: outputFilename            ! name of the output file
  ! output
  integer(i4b),intent(out)       :: err                       ! error code
  character(*),intent(out)       :: message                   ! error message
  ! local
  integer(i4b),parameter         :: gridFileUnit=77           ! grid file unit
  integer(i4b)                   :: iTrial,jTrial             ! loop over the grid
  real(dp), dimension(nLayers)   :: mLayerVolFracLiqLocal     ! volumetric fraction of liquid water (-)
  real(dp), dimension(nLayers)   :: mLayerVolFracIceLocal     ! volumetric fraction of ice (-)
  real(dp), dimension(nLayers)   :: mLayerVolFracWatLocal     ! volumetric fraction of total water (-)
  real(dp)                       :: scalarCanopyLiqLocal      ! mass of canopy liquid (kg m-2)
  real(dp)                       :: scalarCanopyIceLocal      ! mass of canopy liquid (kg m-2)
  real(dp)                       :: xDiff(nState)             ! difference in the state vector
  real(dp)                       :: xDiffScaled(nState)       ! scaled difference in the state vector
  real(qp)                       :: rVecTemp(nState)          ! temporary residual vector (NOTE: quadruple precision)
  real(dp)                       :: rVecScaled(nState)        ! scaled residual vector
  real(dp)                       :: gradScaledLocal(nState)   ! scaled gradient
  real(dp)                       :: grad(nState)              ! gradient
  ...
  integer(i4b)                   :: ixVar1                    ! 1st variable
  integer(i4b)                   :: ixVar2                    ! 2nd variable
  ! initialize error control
  ...
  open(unit=gridFileUnit,file=outputFilename)

  ! loop through trial values
  do jTrial=-50,50
   do iTrial=-50,50

    ! define the difference vector (scaled)
    xDiffScaled(ixVar1) = abs(newtStep(ixVar1))*real(iTrial, dp)/1000._dp
    xDiffScaled(ixVar2) = abs(newtStep(ixVar2))*real(jTrial, dp)/1000._dp
    ...
    ! define the new state vector 
    stateVecNew = stateVecTrial + xInc + xDiff
    ...
    if(fOld < 0.5_dp*veryBig)then
     ...
     ! (matrix multiplication of the scaled Jacobian and the scaled iteration increment)
     select case(ixSolve) ! check if full Jacobian or band-diagonal matrix
      case(ixFullMatrix); tempVec = matmul(oldJac,xDiffScaled)  ! full Jacobian matrix
      case(ixBandMatrix) ! band-diagonal matrix
       tempVec(:) = 0._dp
       do iJac=1,nState  ! (loop through state variables)
        do iState=max(1,iJac-ku),min(nState,iJac+kl)
         tempVec(iState) = tempVec(iState) + xDiffScaled(iJac)*oldJac(kl+ku+1+iState-iJac,iJac)
        end do
       end do
      case default; err=20; message=trim(message)//'unable to identify option for the type of matrix'
     end select  ! (option to solve the linear system A.X=B)

     ! (predicted function evaluation)
     tempVec = tempVec + rVecScaled  ! compute the difference from the residual vector
     fPred   = 0.5_dp*dot_product(tempVec,tempVec)
    ...
    ! compute the gradient of the function vector
    call computeGradient(aJacScaled,rVecScaled,gradScaledLocal,grad,err,cmessage)
    if(err/=0)then; message=trim(message)//trim(cmessage); return; endif  ! (check for errors)

    ! print objective function
    write(gridFileUnit,'(100(e20.10,1x))') xDiffScaled(ixVar1), xDiffScaled(ixVar2), stateVecNew(ixVar1), stateVecNew(ixVar2), fActual, fPred, &
                                           gradScaledLocal(ixVar1), gradScaledLocal(ixVar2), gradScaled(ixVar1), gradScaled(ixVar2)

   end do  ! iTrial
  end do  ! jTrial

  ! add the state vector and iteration increment
  write(gridFileUnit,'(2(f15.5,1x))') stateVecTrial(ixVar1), stateVecTrial(ixVar2)
  write(gridFileUnit,'(2(f15.5,1x))') xInc(ixVar1), xInc(ixVar2)

  ! close the file
  close(gridFileUnit)

  ! deallocate space
  deallocate(aJac,aJacScaled,stat=err)
  if(err/=0)then; err=20; message=trim(message)//'unable to deallocate Jacobian matrices'; return; endif
